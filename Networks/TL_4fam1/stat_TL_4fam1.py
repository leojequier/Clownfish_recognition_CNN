...
def Mkgrid():
    ...
    inputs, classes, path = next(iter(dataset_loader_t['train']))
    print(type(inputs))
    print(inputs.size())
...
#----------------------------------------------------------------------------
#instantiate a network based on resnet 18 with pretrained parameter.
# Parameters of newly constructed modules have requires_grad=True by default
# so loop over parameter and prevent their modification.
model_conv = torchvision.models.resnet18(pretrained=True)
for param in model_conv.parameters():
    param.requires_grad = False

# count the number of feature in the last layer of the model
num_ftrs = model_conv.fc.in_features

# add a additionnal fully connected layer with number of output = number of class
...
model_conv.load_state_dict(torch.load("trainedTL_4fam.pt"))
...
pred_correct = list(0. for i in range(4))
pred_total = list(0. for i in range(4))
with torch.no_grad():
    ngrid = 1
...
for i in range(4):
    ...
    print('Specificity of %5s : %d/%d = %2d %%' % (
